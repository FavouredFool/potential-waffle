...
public class Enemy : MonoBehaviour
{
    ...
    [SerializeField] int _minGuts;
    [SerializeField] int _maxGuts;
    [SerializeField] GameObject _gooBlueprint;
    [SerializeField] GameObject _gutsBlueprint;
    ...
    void Update()
    {
        if (_currentHealth <= 0)
        {
            // Kill, spawn goo and resources
            
            for (int i = 0; i < Random.Range(5, 9); i++) 
            {
                Instantiate(_gooBlueprint, transform.position, transform.rotation);
            }

            for (int i = 0; i < Random.Range(_minGuts, _maxGuts+1); i++) 
            {
                Instantiate(_gutsBlueprint, transform.position, transform.rotation);
            }
